...
인벤 게시판에서 **내 글 / 내 댓글**을 빠르게 정리하고, 일부 게시판에서는 **No인장 필터**, **메이플 전투력 표시**, **주사위 찾기**를 사용할 수 있는 Chrome/Firefox 확장 프로그램입니다.

## 주요 기능

- **내 글 삭제** (`?my=post`) — 체크박스로 여러 글을 선택하거나 댓글 없는 글을 한 번에 삭제
- **내 댓글 삭제** (`?my=opi`) — 각 글의 댓글을 불러와 내 댓글만 선택 삭제
- **빠른 일괄 처리** — 글 삭제와 댓글 조회/삭제를 여러 건씩 병렬 처리
...
- **비추천 수 표시** — 게시글 목록에서 추천 수 옆에 비추천 수를 표시
- **전투력 표시** — 메이플 인벤 전역의 게시글 댓글에서 인증 인장 있는 닉네임 옆에 전투력 구간, 최근 갱신일, 시그니쳐 업적 아이콘을 표시
- **가려진 댓글 수 표시** — 게시글 내부에서 No인장/전투력 미만 가리기로 숨겨진 댓글 수를 표시
...
삭제와 댓글 조회는 여러 건씩 병렬 처리됩니다. 삭제 실패가 발생할 수 있으므로 완료 메시지의 성공/실패 건수를 확인해 주세요.
...
게시글 목록에서는 추천 수 옆에 비추천 수를 `/ 숫자` 형태로 함께 표시합니다.

`전투력 표시`를 켜면 메이플 인벤 전역의 게시글 댓글에서 인증 인장이 있는 닉네임 옆에 전투력 구간을 표시합니다. 전투력에 마우스를 올리면 캐릭터 메이플 프로필의 최근 갱신일을 보여줍니다. 캐릭터 메이플 프로필에 시그니쳐 업적이 있으면 전투력 옆에 업적 아이콘도 함께 표시하고, 아이콘에 마우스를 올리면 업적명과 설명을 보여줍니다. No인장 댓글에는 전투력을 표시하지 않습니다. 필요하면 `미만 가리기`를 켜고 기준 전투력을 선택해 해당 전투력 미만 댓글을 숨길 수 있습니다.

게시글 내부에서는 `No인장 가리기`의 댓글 가리기 또는 `전투력 미만 가리기`가 켜져 있을 때만 숨겨진 댓글 수를 표시합니다. 두 기능이 모두 꺼져 있으면 카운터를 표시하지 않으며, 켜진 기능의 항목만 개별 개수로 보여줍니다.
...
- **전투력 최근 갱신일**은 캐릭터 메이플 프로필에서 갱신일 정보를 불러올 수 있는 경우에만 표시됩니다.
