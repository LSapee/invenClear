...
- **전투력 표시** — 메이플 인벤 전역의 게시글 댓글에서 인증 인장 있는 닉네임 옆에 전투력 구간과 시그니쳐 업적 아이콘을 표시
...
`전투력 표시`를 켜면 메이플 인벤 전역의 게시글 댓글에서 인증 인장이 있는 닉네임 옆에 전투력 구간을 표시합니다. 캐릭터 메이플 프로필에 시그니쳐 업적이 있으면 전투력 옆에 업적 아이콘도 함께 표시하고, 아이콘에 마우스를 올리면 업적명과 설명을 보여줍니다. No인장 댓글에는 전투력을 표시하지 않습니다. 필요하면 `미만 가리기`를 켜고 기준 전투력을 선택해 해당 전투력 미만 댓글을 숨길 수 있습니다.
